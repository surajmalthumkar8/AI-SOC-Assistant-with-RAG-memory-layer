...
from comprehensive_analyzer import full_investigation
...
class AlertMonitor:
    ...
    async def _process_alert(self, event: Dict):
        ...
        try:
            ...
            # Step 2: For P1/P2 or escalate decisions, run full 6-step investigation
            if priority in ["P1", "P2"] or decision == "escalate":
                log("High priority - running full investigation pipeline")
                investigation_report = await full_investigation(event)
                self.stats["llm_analyses"] += 1
                self.stats["enrichments_performed"] += 1

                classification = investigation_report.get("analysis", {}).get("classification", "unknown")
                if "true_positive" in classification.lower():
                    self.stats["true_positives"] += 1
                elif "false_positive" in classification.lower():
                    self.stats["false_positives"] += 1

                # full_investigation() already writes to RAG + Splunk via write-back
                log("Investigation complete", {
                    "id": investigation_report.get("investigation_id"),
                    "classification": classification,
                    "writeback": investigation_report.get("splunk_writeback", False)
                })
...
class BatchProcessor:
    ...
    async def process_all_pending(self) -> Dict:
        ...
        for i, event in enumerate(events):
            ...
            try:
                report = await full_investigation(event)
                reports.append({
                    "event": event,
                    "investigation_id": report.get("investigation_id"),
                    "classification": report.get("analysis", {}).get("classification"),
                    "severity": report.get("analysis", {}).get("severity"),
                    "splunk_writeback": report.get("splunk_writeback", False)
                })
            except Exception as e:
