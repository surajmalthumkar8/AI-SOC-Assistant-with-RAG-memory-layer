...
class SplunkMCPServer:
    ...
    async def ingest_event(self, index: str, sourcetype: str, event_data: Dict) -> Dict:
        """Ingest event to Splunk via /services/receivers/simple.
        Sends index/sourcetype as query params and the event as raw body content."""
        ...
        async with httpx.AsyncClient(verify=False, timeout=30.0) as client:
            kwargs = {"params": params, "content": body}
            if self.auth_header:
                kwargs["headers"] = self.auth_header
            else:
                kwargs["auth"] = self.auth
            ...
            if response.status_code == 200:
                log(f"Event ingested to {index}/{sourcetype}")
                return {"status": "ingested", "index": index, "sourcetype": sourcetype}
            else:
                raise Exception(f"Splunk ingest failed: {response.status_code} - {response.text[:200]}")
